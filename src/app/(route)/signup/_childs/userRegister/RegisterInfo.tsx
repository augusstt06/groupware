...
export default function RegisterInfo(props: RegisterInfoTypeProps) {
  ...
  const phoenNumValidate = () => {
    ...
    const isPhoneNumValidate: boolean = inputValidate(validateProps)
  ...
  const checkEmail = () => {
    ...
    const isEmailValidate: boolean = inputValidate(validateProps)
    let reducerProps
    if (!isEmailValidate) {
      reducerProps = {
        isCheck: false,
  ...
  const inputList = [
    ...
    },
    {
      ...
      title: REGISTER_PWD_EN,
      useInput: pwdInput,
      inputViewType: props.isPwdView === true ? 'text' : 'password',
      handleViewType: () => {
    ...
    },
    {
      ...
      inputViewType: props.isPwdConfirmView === true ? 'text' : 'password',
      handleViewType: () => {
        // eslint-disable-next-line @typescript-eslint/strict-boolean-expressions
  ...
  useEffect(() => {
    ...
    const isPwdValidate: boolean = inputValidate({
      inputData: pwdInput.value,
      dataType: VALIDATE_PWD_TYPE,
    })
    if (pwdInput.value !== '') {
      if (!isPwdValidate) {
        setErrorMsg(
