...
import { createOrgTeamReducer } from '@/app/store/reducers/orgInfoReducer'
import { type NextStepOrgTeamProps, type RegisterOrgBtnProps } from '@/app/types/ui/btnTypes'

export function OrgTeamBtn(props: RegisterOrgBtnProps) {
  // 버튼 클릭시 현재 작성한 팀의 정보가 오른쪽의 팀 현황ㅇ로 이동하고 밸류 초기화
  const dispatch = useAppDispatch()
  const currentTeamValue = useAppSelector((state) => state.orgInfo.currentTeam)
  const handleClick = () => {
    if (props.title === 'Confirm Team') {
      dispatch(createOrgTeamReducer(currentTeamValue))
      alert('팀 생성이 완료되었습니다.')
    }
  }
  return (
